...
@WebServlet("/api/v1.0/UploadFileServlet")
public class UploadFileServlet extends HttpServlet {
	...
	/** The Constant log. */
	private static final Logger LOGGER = LoggerFactory.getLogger(UploadFileServlet.class.getName());
...
//	private static final String PATH = "C:/Users/Emanuel/wildfly-21.0.2.Final/standalone/deployments/";
	...
	private static final String UUID_EMPRESA = "uuidEmpresa";
...
//	                    doctype = DocTypeEnum.valueOf(docType);
	                ...
	                }else if (fieldName.equalsIgnoreCase(UUID_EMPRESA)) {
	                     uuidEmpresa = item.getString();
	                    LOGGER.info("[UploadFileServlet - doService] - uuidVenta:{}", uuidVenta);
	                }
	            }
	        }
	        ...
	        if (uuidVenta == null) {
	        	if (uuidEmpresa == null) throw new IllegalArgumentException("uuidEmpresa cannot be null");
			}
	        ...
	        docVo.setPath(fullpath);
	        docVo.setFilename(String.valueOf(System.currentTimeMillis())); 
	        docVo.setFilenameOriginal(filenameOriginal);
	        docVo.setFilepath(fullpath);
	        
//	        docVo.setUrl("/api/v1.0/DownloadFileServlet?file=" + filename);
	        
	        String downloadUrl = downloadFileServlet.generateDownloadUrl(docVo);
	        docVo.setUrl(downloadUrl);
	        
	        docVo.setFechaCreacion(Calendar.getInstance());
	        docVo.setFechaModificacion(Calendar.getInstance());
	        docVo.setUser(authUser.getUsername());
	        
	        VentaVo ventaVo = null;
	        if (uuidVenta != null) {
	        	 ventaVo = ventasService.loadByuuid(uuidVenta);
			}
	        
	        EmpresaVo empresaVo = null;
	        if (uuidEmpresa != null) {
	        	empresaVo = empresaService.loadByUuId(uuidEmpresa);
			}
	        
	        if (ventaVo == null && empresaVo == null) {
	            throw new IllegalArgumentException("VentaEntity not found for UUID: " + uuidVenta);
	        }
	        if (ventaVo != null) {
	        	 docVo.setIdVenta(ventaVo.getIdVenta());
			}
	       if (empresaVo != null) {
	    	    docVo.setIdEmpresa(empresaVo.getIdEmpresa());
		}
